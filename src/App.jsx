      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6 mb-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h1 className="text-3xl font-bold text-slate-900">Sistema de Análise de Produção Industrial</h1>
          </div>
          ...
          <>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6 mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                ...
                {/* Prensa Filter */}
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-2">Máquina (Prensa)</label>
                  <select
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-slate-50">
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Ano/Mês</th>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Máquina</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Prod. Máxima</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-slate-200">
                    {filteredData.map((item, index) => (
                      <tr 
                        key={item.id} 
                        className={`hover:bg-slate-50 ${item['Produção Máxima'] === stats?.maxProduction ? 'bg-green-50 border-green-200' : ''}`}
                      >
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-900">
                          {item.Ano}/{getMonthName(item.Meses)}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-900">{item.Grupo}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900">{item.Produto}</td>
                        <td className="px-6 py-4 text-sm text-slate-900">{item.Descricao}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-900">{item.PRENSA}</td>
                        <td className={`px-6 py-4 whitespace-nowrap text-sm font-semibold ${item['Produção Máxima'] === stats?.maxProduction ? 'text-green-600' : 'text-slate-900'}`}>
        ...
        {data.length === 0 && !isLoading && (
          <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-12 text-center">
            ...
            <h3 className="text-lg font-semibold text-slate-900 mb-2">Carregue sua planilha para começar</h3>
